...
pub fn get_inventory(_: ()) -> ExternResult<GetInventoryOutput> {
  ...
  let templates = links.iter().map(|link| link.target.clone().into()).collect();
  /// Get svg_markers
  let base = get_svg_marker_path().path_entry_hash()?;
  let links = get_links(base, None)?;
  let svg_markers = links.iter().map(|link| link.target.clone().into()).collect();
  /// Get emoji_groups
  let base = get_emoji_group_path().path_entry_hash()?;
  let links = get_links(base, None)?;
  let emoji_groups = links.iter().map(|link| link.target.clone().into()).collect();
  /// Get spaces
