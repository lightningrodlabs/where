...
export default (orchestrator) => {
  ...
  orchestrator.registerScenario('test profile zomes', async (s, t) => {
    ...
    const [alice_where_happ, bob_where_happ] = await installAgents(conductor,  ["alice", "bobbo"])
    const [alice] = alice_where_happ.cells
    const [bobbo] = bob_where_happ.cells

    // Trigger init to run in both zomes
    await alice.call('where', 'list_channels', { category: "General" });
    await bobbo.call('where', 'list_channels', { category: "General" });
