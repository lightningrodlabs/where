...
import { EmojiGroupEntry } from "../types";
...
export class WhereEmojiGroupDialog extends ScopedElementsMixin(LitElement) {
  ...
  private async handleOk(e: any) {
    ...
    this.dispatchEvent(
      new CustomEvent("emoji-group-added", {
        detail: newGroupEh,
        bubbles: true,
        composed: true,
      })
  ...
  render() {
    ...
    const emojis = Object.entries(this._currentUnicodes).map(
      ([key, unicode]) => {
        return html`
          <mwc-icon-button
            class="unicode-button"
            @click=${(e: any) => this.handleEmojiButtonClick(unicode)}
            >${unicode}</mwc-icon-button
          >
        `;
      }
    );
    ...
    return html`
      <mwc-dialog
        id="emoji-group-dialog"
        heading="New Emoji Group"
      ...
      >
        ...
        <div style="margin:10px 0px 10px 0px;">
          <div class="unicodes-container">${emojis}</div>
        </div>
        <!-- Emoji Picker -->
        ...
        <mwc-button slot="secondaryAction" dialogAction="cancel"
          >cancel</mwc-button
        >
        <mwc-button slot="secondaryAction" @click=${this.handleResetGroup}
          >reset</mwc-button
        >
      </mwc-dialog>
    `;
