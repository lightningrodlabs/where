...
export class WherePlayDialog extends ScopedElementsMixin(LitElement) {
  ...
  get emojiGroupDialogElem(): WhereEmojiGroupDialog {
    return this.shadowRoot!.getElementById(
      "emoji-group-dialog"
    ) as WhereEmojiGroupDialog;
  }

  get svgMarkerDialogElem(): WhereSvgMarkerDialog {
    return this.shadowRoot!.getElementById(
      "svg-marker-dialog"
    ) as WhereSvgMarkerDialog;
  }
  ...
  loadPreset(spaceEh: EntryHashB64) {
    ...
    this._uiField.value = originalPlay.space.meta.ui
      ? JSON.stringify(originalPlay.space.meta!.ui)
      : "[\n]";
    // - Markers
    this._markerTypeField.value =
      MarkerType[originalPlay.space.meta.markerType];
    ...
    this._predefinedTagsField.value =
      originalPlay.space.meta.predefinedTags.join();
    if (originalPlay.space.meta.markerType == MarkerType.Tag) {
      this._tagChk.disabled = true;
      this._tagVisibleChk.disabled = true;
      this.tagChkLabel.label =
        "Display tag on surface (Tag Marker type selected)";
    }
    // - Iterations
    this.noStopRadioElem.checked =
      originalPlay.space.meta.sessionCount == 0 ||
      originalPlay.space.meta.sessionCount == 1;
    this.fixedStopRadioElem.checked = originalPlay.space.meta.sessionCount > 1;
    this.generativeStopRadioElem.checked =
      originalPlay.space.meta.sessionCount < 0;
    this._sessionCountField.value =
      originalPlay.space.meta.sessionCount.toString();
    this._sessionLabelsField.value =
      originalPlay.space.meta.sessionLabels.join();
  ...
  updated(changedProperties: any) {
    ...
    if (this._canvas) {
      ...
      try {
        ...
        console.log("render canvas failed");
      }
    }
    /** Setup emoji picker */
    let emojiPickerElem = this.shadowRoot!.getElementById(
      "emoji-picker"
    ) as Picker;
    if (emojiPickerElem) {
  ...
  private async handleOk(e: any) {
    ...
    let locationsTab = this.shadowRoot!.getElementById(
      "locations-tab"
    ) as SlTab;
    let iterationsTab = this.shadowRoot!.getElementById(
      "iterations-tab"
    ) as SlTab;
    ...
    if (this.fixedStopRadioElem.checked) {
      ...
      if (this._sessionLabelsField.value != "") {
        ...
        const isStopLabelsValid =
          strs.length == parseInt(this._sessionCountField.value);
    ...
    // - Marker
    this._currentMeta.markerType =
      MarkerType[this.determineMarkerType() as keyof typeof MarkerType];
    this._currentMeta.singleEmoji = singleEmojiElem
      ? singleEmojiElem.innerText
      : "";
    // - Tag
    this._currentMeta.multi = this._multiChk.checked;
    this._currentMeta.predefinedTags =
      this._predefinedTagsField.value.split(",");
    // - Iterations
    this._currentMeta.sessionCount = this._sessionCountField.value
      ? parseInt(this._sessionCountField.value)
      : 2;
    ...
    if (this.fixedStopRadioElem.checked) {
      if (
        this._currentMeta.sessionLabels.length == 1 &&
        this._currentMeta.sessionLabels[0] == ""
      ) {
  ...
  resetAllFields(canResetName?: boolean) {
    ...
    // set first tab
    let tabGroup = this.shadowRoot!.getElementById(
      "space-tab-group"
    ) as SlTabGroup;
    ...
    for (let placeholder of this._currentPlaceHolders) {
      let field = this.shadowRoot!.getElementById(
        placeholder + "-gen"
      ) as TextField;
  ...
  generateSurface() {
    ...
    for (let placeholder of this._currentPlaceHolders) {
      const txtfield = this.shadowRoot!.getElementById(
        placeholder + "-gen"
      ) as TextField;
  ...
  renderTemplateFields() {
    ...
    return html`${this._currentPlaceHolders.map(
      (name) => html` <mwc-textfield
        outlined
        id="${name}-gen"
        label="${name}"
        value=""
        @input="${name === "ImageUrl" ? this.handleImageUrlField : null}"
      ></mwc-textfield>`
    )}`;
  }
  ...
  handleImageUrlField(e: any) {
    const imgComp = this.shadowRoot!.getElementById(
      "ImageUrl-gen"
    ) as TextField;
    try {
  ...
  renderMarkerTypePreview(markerType: MarkerType) {
    ...
    locMeta.img = this.currentProfile
      ? this.currentProfile!.fields.avatar
      : "42";
    switch (markerType) {
    ...
    locMeta.color = this.currentProfile
      ? this.currentProfile!.fields.color
      : "blue";
    locMeta.authorName = this.currentProfile
      ? this.currentProfile!.nickname
      : "(none)";
    return html`<div id="marker-preview" class="location-marker">
      ${renderMarker(locMeta, false)}
    </div>`;
  }
  ...
  renderSurfacePreview() {
    ...
    const ratio: number =
      surface.size && surface.size.x > 0 ? surface.size.y / surface.size.x : 1;
    const w: number = 200;
    const h: number = 200 * ratio;
    let uiItems = html``;
    try {
      const ui = JSON.parse(this._uiField.value);
      uiItems = this._uiField
        ? renderUiItems(ui, w / surface.size.x, h / surface.size.y)
        : html``;
    } catch (e) {
    ...
    if (surface.html) {
      preview = html`
        ${uiItems}
        <div
          style="width: ${w}px; height: ${h}px; margin-bottom:5px;"
          id="surface-preview-div"
        >
  ...
  handleIterationTypeChange(e: any) {
    const oneIterationRadio = this.shadowRoot!.getElementById(
      "no-stop-radio"
    ) as Radio;
    const manyIterationRadio = this.shadowRoot!.getElementById(
      "fixed-stop-radio"
    ) as Radio;
    if (oneIterationRadio.checked) {
      this._currentMeta.sessionCount = 0;
    } else {
      this._currentMeta.sessionCount = manyIterationRadio!.checked
        ? parseInt(manyIterationRadio.value)
        : -parseInt(manyIterationRadio.value);
    }
    this.requestUpdate();
  }
    ...
    switch (this._markerTypeField.value) {
      ...
      case MarkerType[MarkerType.EmojiGroup]:
        if (Object.keys(this._emojiGroups.value)[0]) {
          this._currentMarker = {
            svg: Object.keys(this._emojiGroups.value)[0],
          };
        }
        break;
      default:
  ...
  handleEmojiGroupSelect(e?: any) {
    ...
    let emojiGroupField = this.shadowRoot!.getElementById(
      "emoji-group-field"
    ) as Select;
  ...
  render() {
    ...
    if (!this._currentTemplate || this._currentTemplate.surface === "") {
      ...
      this._currentTemplate = this._templates.value[firstTemplate]
        ? this._templates.value[firstTemplate]
        : null;
    ...
    switch (markerType) {
      case MarkerType.SingleEmoji:
        let emoji =
          this._currentMeta.singleEmoji != ""
            ? this._currentMeta.singleEmoji
            : "😀";
        maybeMarkerTypeItems = html`
          <span
            id="space-unicodes"
            style="margin-top:20px;font-size:${EMOJI_WIDTH}px;display:inline-flex;"
            >${emoji}</span
          >
          <mwc-icon-button
            icon="edit"
            style="margin-top:10px;"
            @click=${() => this.openEmojiDialog(this._currentMeta.singleEmoji)}
          ></mwc-icon-button>
      ...
      case MarkerType.EmojiGroup:
        ...
        const groups = Object.entries(this._emojiGroups.value).map(
          ([key, emojiGroup]) => {
            ...
            return html`
              <mwc-list-item
                class="emoji-group-li"
                value="${emojiGroup.name}"
                .selected=${key == marker_eh}
              >
                ${emojiGroup.name}
              </mwc-list-item>
            `;
        ...
        if (this._currentMarker && "emojiGroup" in this._currentMarker) {
          const currentGroup =
            this._emojiGroups.value[this._currentMarker.emojiGroup];
          unicodes = currentGroup.unicodes.reduce(
        ...
        maybeMarkerTypeItems = html`
          <mwc-icon-button
            icon="add_circle"
            style="margin-top:10px;"
            @click=${() => this.openEmojiGroupDialog(null)}
          ></mwc-icon-button>
          <mwc-icon-button
            icon="edit"
            style="margin-top:10px;"
            @click=${() => this.openEmojiGroupDialog(marker_eh)}
          ></mwc-icon-button>
          <mwc-select
            id="emoji-group-field"
            required
            style=""
            label="Subset"
            @closing=${(e: any) => {
              e.stopPropagation();
              this.handleEmojiGroupSelect(e);
            }}
          >
            ${groups}
          </mwc-select>
          ...
          <div
            id="space-unicodes"
            class="unicodes-container"
            style="font-size:28px;margin-top:10px;padding-top:8px;padding-bottom:2px"
          >
            ${unicodes}
          </div>
          <!-- </div> -->
      ...
      case MarkerType.SvgMarker:
        ...
        /** Build marker list */
        if (
          (!this._currentMarker || "emojiGroup" in this._currentMarker) &&
          Object.keys(this._svgMarkers.value).length > 0
        ) {
          this._currentMarker = { svg: Object.keys(this._svgMarkers.value)[0] };
        }
        const markers = Object.entries(this._svgMarkers.value).map(
          ([key, svgMarker]) => {
            ...
            return html`
              <mwc-list-item
                class="svg-marker-li"
                value="${svgMarker.name}"
                .selected=${key == marker_eh}
              >
                ${svgMarker.name} ${currentMarker}
              </mwc-list-item>
            `;
          }
        );
        ...
        maybeMarkerTypeItems = html`
          <mwc-icon-button
            icon="add_circle"
            style="margin-top:10px;"
            @click=${() => this.openSvgMarkerDialog(null)}
          ></mwc-icon-button>
          <mwc-icon-button
            icon="edit"
            style="margin-top:10px;"
            @click=${() => this.openSvgMarkerDialog(marker_eh)}
          ></mwc-icon-button>
          <mwc-select
            id="svg-marker-field"
            required
            style="display:inline-flex;width:230px;"
            label="Name"
            @closing=${(e: any) => {
              e.stopPropagation();
              this.handleSvgMarkerSelect(e);
            }}
          >
            ${markers}
          </mwc-select>
    ...
    const isFixedSelected =
      this.fixedStopRadioElem && this.fixedStopRadioElem.checked;
    const isGenSelected =
      this.generativeStopRadioElem && this.generativeStopRadioElem.checked;

    /** Main Render */
    return html`
<mwc-dialog id="space-dialog" heading="NEW SPACE" @closing=${
  ...
  <sl-tab-panel name="general">
    <div id="thumbnail">
      ...
      <mwc-button dense unelevated style="display:block;margin-left:45px;margin-bottom:20px;" @click=${
        this.handlePreview
      }>preview</mwc-button>
    ...
    <mwc-select fixedMenuPosition required id="template-field" label="Template" @select=${
        ...
        ${Object.entries(this._templates.value).map(
          ([key, template]) => html`
            <mwc-list-item
              ...
              .selected=${this._templates.value[key].name ===
  ...
  <sl-tab-panel name="locations">
    ...
    <mwc-select label="Type" id="marker-select" required @closing=${(
      e: any
    ) => {
      e.stopPropagation();
      this.handleMarkerTypeSelect(e);
    }}>
      <mwc-list-item selected value="${
        MarkerType[MarkerType.Avatar]
      }">Avatar ${this.renderMarkerTypePreview(
      MarkerType.Avatar
    )}</mwc-list-item>
      <mwc-list-item value="${
        MarkerType[MarkerType.Initials]
      }">Initials ${this.renderMarkerTypePreview(
      MarkerType.Initials
    )}</mwc-list-item>
      <mwc-list-item value="${
        MarkerType[MarkerType.SvgMarker]
      }">Colored SVG ${this.renderMarkerTypePreview(
      MarkerType.SvgMarker
    )}</mwc-list-item>
      <mwc-list-item value="${
        MarkerType[MarkerType.SingleEmoji]
      }">Predefined Emoji ${this.renderMarkerTypePreview(
      MarkerType.SingleEmoji
    )}</mwc-list-item>
      <mwc-list-item value="${
        MarkerType[MarkerType.EmojiGroup]
      }">Emoji subset ${this.renderMarkerTypePreview(
      MarkerType.EmojiGroup
    )}</mwc-list-item>
      <mwc-list-item value="${
        MarkerType[MarkerType.AnyEmoji]
      }">Any Emoji ${this.renderMarkerTypePreview(
      MarkerType.AnyEmoji
    )}</mwc-list-item>
      <mwc-list-item value="${
        MarkerType[MarkerType.Tag]
      }">Tag ${this.renderMarkerTypePreview(MarkerType.Tag)}</mwc-list-item>
    </mwc-select>
    ...
    <mwc-formfield label="Enable marker tagging">
      <mwc-checkbox id="tag-chk" @click=${
        this.handleCanTagClick
      }></mwc-checkbox>
    </mwc-formfield>
    <mwc-formfield id="tag-chk-lbl" label="Display tag on surface" style="margin-left:10px">
      <mwc-checkbox id="tag-visible-chk" @click=${
        this.handleCanTagBeVisibleClick
      }></mwc-checkbox>
    </mwc-formfield>
    </mwc-formfield>
    <mwc-textfield outlined style="margin-left:25px" type="text" .disabled="${!this
      ._currentMeta.canTag}"
  ...
  <sl-tab-panel name="iterations">
    ...
    <mwc-formfield label="None">
      <mwc-radio id="no-stop-radio" name="a" value="fixed" @change=${
        this.handleIterationTypeChange
      }></mwc-radio>
    </mwc-formfield>
    <!-- Predefined Iterations -->
    <mwc-formfield label="Predefined">
      <mwc-radio id="fixed-stop-radio" name="a" value="fixed" @change=${
        this.handleIterationTypeChange
      }></mwc-radio>
    </mwc-formfield>
    <div id="fixed-stops-div" style="margin-left:30px;margin-top:5px;">
      <mwc-textfield id="stop-count-field" outlined type="number" style="margin-right:10px;width:90px;"
                     ...
                     value="${
                       this._currentMeta.sessionCount
    ...
    <mwc-formfield label="One per day">
      <mwc-radio id="generative-stop-radio" name="a" value="generative" @change=${
        this.handleIterationTypeChange
      }></mwc-radio>
    </mwc-formfield>
    <div id="generative-stops-div" style="margin-left:20px;min-height:50px;">
      ...
      <mwc-checkbox id="can-modify-past-chk" .disabled="${!isGenSelected}" @click=${
      this.handleCanModifyPastClick
    }></mwc-checkbox>
      </mwc-formfield>
    </div>
  </sl-tab-panel>
  ...
  <sl-tab-panel name="advanced">
      ...
      <mwc-textarea type="text" @input=${() =>
        (
          this.shadowRoot!.getElementById("ui-field") as TextArea
        ).reportValidity()}
  ...
  <!-- Dialog buttons -->
  <mwc-button id="primary-action-button" raised slot="primaryAction" @click=${
    this.handleOk
  }>ok</mwc-button>
  ...
  <!-- Inner dialogs -->
  <where-emoji-dialog id="emoji-dialog" @emoji-selected=${(e: any) =>
    this.handleEmojiSelected(e.detail)}></where-emoji-dialog>
  <where-emoji-group-dialog id="emoji-group-dialog" @emoji-group-added=${(
    e: any
  ) => this.handleGroupAdded(e.detail)}></where-emoji-group-dialog>
  <where-svg-marker-dialog id="svg-marker-dialog" @svg-marker-added=${(
    e: any
  ) => this.handleSvgMarkerAdded(e.detail)}></where-svg-marker-dialog>
</mwc-dialog>
`;
  }
  ...
  handleGroupAdded(eh: EntryHashB64) {
    ...
    const unicodes = emojiGroup.unicodes.reduce(
      (prev, cur, idx) => (prev += cur)
    );
    let unicodeContainer = this.shadowRoot!.getElementById("space-unicodes");
    unicodeContainer!.innerHTML = unicodes;
  ...
  handleSvgMarkerAdded(eh: EntryHashB64) {
    ...
    let svgMarkerField = this.shadowRoot!.getElementById(
      "svg-marker-field"
    ) as Select;
