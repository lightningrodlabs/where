...
import { ludothequeContext, whereContext } from "../context";
import {
  EMOJI_WIDTH,
  renderMarker,
  renderSvgMarker,
...
export class WhereSpaceDialog extends ScopedElementsMixin(LitElement) {
  ...
  loadPreset(spaceEh: EntryHashB64) {
    ...
    this._uiField.value = originalSpace!.meta.ui
      ? JSON.stringify(originalSpace.meta!.ui)
      : "[\n]";
    // - Markers
...
<mwc-dialog id="space-dialog" heading="NEW SPACE" @closing=${
  ...
  <where-emoji-group-dialog id="emoji-group-dialog" .store="${
    this._store
  }" @emoji-group-added=${(e: any) =>
      this.handleGroupAdded(e.detail)}></where-emoji-group-dialog>
  <where-svg-marker-dialog id="svg-marker-dialog" .store="${
    this._store
  }" @svg-marker-added=${(e: any) =>
